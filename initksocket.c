...
// Global variables
int shmid_SM = -1;           // Shared memory ID for KTPSocketEntry SM
int shmid_sock_info = -1;    // Shared memory ID for SOCK_INFO
KTPSocketEntry *SM = NULL;   // Pointer to the shared memory segment for KTPSocketEntry SM
SOCK_INFO *sock_info = NULL; // Pointer to the shared memory segment for SOCK_INFO
sem_t *Sem1 = NULL;          // Semaphore for synchronization (used for k_socket and k_bind calls)
sem_t *Sem2 = NULL;          // Semaphore for synchronization (used for k_socket and k_bind calls)
...
void *thread_R()
{
    ...
    while (1)
    {
        ...
        // if timeout occurs, reset the socket set, to include any new KTP sockets that have been formed
        if (activity == 0)
        {
            // reset the timer
            timeout.tv_sec = T;
            ...
            if (FD_ISSET(SM[i].udp_socket_id, &temp))
            {
                ...
                if (msg.header.msg_type == 'D')
                {
                    ...
                    else
                    {
                        ...
                        // Message ack;
                        // ack.header.msg_type = 'A';
                        // ack.header.seq_no = (SM[i].recv_window.next_seq_no == 1) ? 255 : SM[i].recv_window.next_seq_no - 1;
                        // sprintf(ack.msg, "%d", SM[i].recv_window.window_size);
                        // sendto(SM[i].udp_socket_id, &ack, sizeof(Message), 0, (struct sockaddr *)&client_addr, addr_len);
                ...
                else if (msg.header.msg_type == 'A')
                {
                    ...
                    for (int k = 0; k < SM[i].send_window.window_size; k++)
                    {
                        ...
                        if (SM[i].send_window.send_buff[new_idx].ack_no == -1)
                            break;
            ...
            { // Check if the socket is allocated
                ...
                if (persistence_timer[i].flag > 0)
                {
                    ...
                    if (difftime(time(NULL), persistence_timer[i].last_time) >= multiplier * T)
                    {
                        ...
                        persistence_timer[i].last_time = time(NULL);
                        if (persistence_timer[i].flag <= 3)
                            persistence_timer[i].flag++; // to maintain exponential backoff waiting time (within an upper limit)
                        continue;
                    }
                }
                ...
                for (int offset = 0; offset < SM[i].send_window.window_size; offset++)
                {
                    ...
                    if (SM[i].send_window.send_buff[idx].ack_no == -1)
                        break;
...
// garbage collector process G to clean up the corresponding entry in the KTP socket if the corresponding process is killed
void *thread_G()
{
    ...
    while (1)
    {
        ...
        // Iterate through the KTP socket entries
        for (int i = 0; i < MAX_KTP_SOCKETS; i++)
        {
            // Check if the socket is allocated
            if (SM[i].socket_alloted <= 0)
...
int main()
{
    ...
    key_t key_SM = ftok("ksocket.h", 'M'); // Generate a key for the shared memory segment
    ...
    // Wait indefinitely to handle k_socket() and k_bind() calls
    while (1)
    {
        // Wait on Sem1
        if (sem_wait(Sem1) == -1)
